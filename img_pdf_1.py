...
label = tk.Label(top, text="Enter no:of images:",
                 bd=5).place(x=50, y=100)  # Label
d = tk.Entry(top, width=10, bd=5)  # Entry box
d.place(x=190, y=100)
a = tk.Button(top, text="Start", width=7, fg='Green',
              command=process_images, bd=3).place(x=300, y=98)  # button1
w = tk.Button(top, text="Exit!", fg='Red', width=10,
              command=Exit, bd=5).place(x=100, y=180)  # button2
kp = tk.Button(top, text="Github</>", fg='Black', width=10,
               bd=5, command=openwb()).place(x=250, y=180)
top.mainloop()
